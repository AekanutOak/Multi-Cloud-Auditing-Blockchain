...
storage_folder = os.path.join(current_path,"CSP-2")

with open(os.path.join(current_path,"../utils/config.json"),"r") as f:
    config = json.loads(f.read())["CSP-2"]
...
@app.route('/')
def index():
    return "Welcome to CSP-2"

@app.route('/upload', methods=['POST'])
def upload_file():
    data = request.get_json()["data"]

    with open(os.path.join(current_path,"./CSP-2/file_track.json"),"r") as f:
        file_track = json.loads(f.read())

    file_track.update(data)

    with open(os.path.join(current_path,"./CSP-2/file_track.json"),"w") as f:
